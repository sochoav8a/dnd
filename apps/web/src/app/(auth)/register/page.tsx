"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import Link from "next/link";
import { signIn } from "next-auth/react";
import { getGqlClient, REGISTER_MUTATION } from "@/lib/graphql";
import { GameIcon } from "@/components/ui/GameIcon";

export default function RegisterPage() {
  const router = useRouter();
  const [form, setForm] = useState({ email: "", username: "", password: "", confirmPassword: "" });
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);

    if (form.password !== form.confirmPassword) {
      setError("Las contraseñas no coinciden");
      return;
    }

    if (form.password.length < 8) {
      setError("La contraseña debe tener al menos 8 caracteres");
      return;
    }

    setLoading(true);
    try {
      const client = getGqlClient();
      await client.request(REGISTER_MUTATION, {
        input: { email: form.email, username: form.username, password: form.password },
      });

      // Auto-login after registration
      await signIn("credentials", {
        email: form.email,
        password: form.password,
        redirect: false,
      });

      router.push("/characters");
      router.refresh();
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Error al registrarse";
      setError(msg.includes("Email already") ? "El email ya está en uso" :
               msg.includes("Username") ? "El nombre de usuario ya está tomado" : msg);
    } finally {
      setLoading(false);
    }
  }

  return (
    <div className="w-full max-w-md animate-slide-up">
      {/* Sigil / crest */}
      <div className="mb-8 flex flex-col items-center text-center">
        <div className="relative mb-5 flex h-16 w-16 items-center justify-center rounded-full border border-parchment-700/70 bg-stone-950 text-parchment-300 shadow-glow">
          <div className="absolute inset-0 rounded-full bg-parchment-500/5 blur-md" />
          <GameIcon kind="raw" slug="scroll-quill" size={28} />
        </div>
        <p className="font-display text-[10px] uppercase tracking-[0.4em] text-parchment-500/80">
          Inscripción al Compendio
        </p>
        <h1 className="mt-2 font-display text-3xl font-semibold uppercase tracking-[0.14em] text-parchment-100 text-balance">
          Forja tu pergamino
        </h1>
        <p className="mt-3 max-w-sm font-serif text-sm italic text-stone-400">
          Declara tu nombre y el archivista lo grabará en tinta imborrable.
        </p>
      </div>

      <form onSubmit={handleSubmit} className="card-hero space-y-5">
        <div className="divider-ornate -mt-1">
          <span className="font-display text-[10px] uppercase tracking-[0.28em] text-parchment-500/80">
            Nuevo aventurero
          </span>
        </div>

        {error && (
          <div
            role="alert"
            className="flex items-start gap-2 rounded-md border border-red-800/60 bg-red-950/50 px-3 py-2 text-sm text-red-200"
          >
            <span className="badge-danger mt-0.5 shrink-0">Error</span>
            <span>{error}</span>
          </div>
        )}

        <div>
          <label htmlFor="email" className="label mb-1.5">Email</label>
          <input
            id="email"
            type="email"
            value={form.email}
            onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))}
            required
            autoComplete="email"
            className="input"
            placeholder="tu@email.com"
          />
        </div>

        <div>
          <label htmlFor="username" className="label mb-1.5">Nombre de héroe</label>
          <input
            id="username"
            type="text"
            value={form.username}
            onChange={(e) => setForm((f) => ({ ...f, username: e.target.value }))}
            required
            minLength={3}
            maxLength={30}
            autoComplete="username"
            className="input"
            placeholder="Thorin_Jr"
          />
          <p className="mt-1 font-serif text-[11px] italic text-stone-500">
            Entre 3 y 30 caracteres. Así te llamarán en el salón de la taberna.
          </p>
        </div>

        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          <div>
            <label htmlFor="password" className="label mb-1.5">Palabra de paso</label>
            <input
              id="password"
              type="password"
              value={form.password}
              onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))}
              required
              minLength={8}
              autoComplete="new-password"
              className="input"
              placeholder="••••••••"
            />
          </div>

          <div>
            <label htmlFor="confirmPassword" className="label mb-1.5">Confirmar</label>
            <input
              id="confirmPassword"
              type="password"
              value={form.confirmPassword}
              onChange={(e) => setForm((f) => ({ ...f, confirmPassword: e.target.value }))}
              required
              autoComplete="new-password"
              className="input"
              placeholder="••••••••"
            />
          </div>
        </div>

        <button
          type="submit"
          disabled={loading}
          className="btn-primary w-full py-2.5 text-sm font-semibold uppercase tracking-[0.14em]"
        >
          {loading ? "Sellando el pergamino…" : "Firmar el pacto"}
        </button>

        <div className="divider" />

        <p className="text-center text-sm text-stone-400">
          ¿Ya tienes pergamino?{" "}
          <Link href="/login" className="link-subtle font-medium text-parchment-300">
            Regresa al umbral
          </Link>
        </p>
      </form>

      <p className="mt-6 text-center font-serif text-[11px] italic text-stone-600">
        SRD 5.1 — pensado para mesa, dados físicos y humo de vela.
      </p>
    </div>
  );
}
